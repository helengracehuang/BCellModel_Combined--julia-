...
#------------------------------------------------
function RateCycBDecay(concentration, New_rates) # V2 in Shokhirev2015
    ans::Float64 = ((New_rates[CYCB, DECAY] * concentration[CDC20P]) + (New_rates[CYCB, BASALDECAY] * (1.0-concentration[CDH1])) + (New_rates[CYCB, PHOSPHORYLATION] * concentration[CDH1]));
    return ans;
end
#------------------------------------------------
function RateCdh1Decay(concentration, New_rates) # V4 in Shokhirev2015
    ans::Float64 = (New_rates[CDH1, DEPHOSPHORYLATION] * ((0.3 * concentration[CYCA]) + (1.0 * concentration[CYCB])));
    return ans;
end
#------------------------------------------------
function RateP27Decay(concentration, New_rates) # V6 in Shokhirev2015
    ans::Float64 = (New_rates[P27, BASALDECAY] + (New_rates[P27, PHOSPHORYLATION] * ((0.5*concentration[CYCA]) + (1.0*concentration[CYCB]) + (0.5*concentration[CYCE]))));
    return ans;
end
#------------------------------------------------
function RateCycEDecay(concentration, New_rates) # V8 in Shokhirev2015
    ans::Float64 = (New_rates[CYCE, DECAY] + (New_rates[CYCE, PHOSPHORYLATION] * (concentration[CYCA] + concentration[CYCE] + 0.05*concentration[CYCB])/(0.15 + concentration[CYCE] + concentration[CYCEP27])));
    return ans;
end
...
function distribute_params(rates, index)
    ...
    copy!(New_rates, rates);
    New_rates[RATE_dict[(index-1) ÷ ST_num + 1][1], RATE_dict[(index-1) ÷ ST_num + 1][2]] *= ST_mod[(index-1) % ST_num + 1];
