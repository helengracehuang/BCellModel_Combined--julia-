...
function computeApoptosisFluxes!(concentration, reactionFlux, Srates, phase)
    @inbounds begin
        ...
        reactionFlux[TBCL2, ACTIVATION] = ((0.3*concentration[NA50] + 0.6*concentration[NC50] + 0.1*concentration[IKK])/BCL2THR)^2.0 / (1+((0.3*concentration[NA50] + 0.6*concentration[NC50] + 0.1*concentration[IKK])/BCL2THR)^2.0) * (1-Srates[TBCL2, ACTIVATION]) + Srates[TBCL2, ACTIVATION];
