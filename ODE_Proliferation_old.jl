...
function computeProlifFluxes!(concentration, reactionFlux, Srates, phase)
    @inbounds begin
        ...
        #-------------------------------------------------
        # A01*. 0 --nNFkB--> tMyc activity : Activation by NFkB
        reactionFlux[TMYC, ACTIVATION] = ((0.45*concentration[NA50] + 0.45*concentration[NC50] + 0.1*reactionFlux[IKK, ACTIVATION])/MYCTHR)^2.0 / (1+((0.45*concentration[NA50] + 0.45*concentration[NC50] + 0.1*reactionFlux[IKK, ACTIVATION])/MYCTHR)^2.0) * (1-Srates[TMYC, ACTIVATION]) + Srates[TMYC, ACTIVATION];
        # A02*. tMyc Act ----> tMyc : Synthesis
        reactionFlux[TMYC, SYNTHESIS] = Srates[TMYC, SYNTHESIS] * reactionFlux[TMYC, ACTIVATION];
        # A03. tMyc ----> 0 : mRNA degradation
        reactionFlux[TMYC, DECAY] = Srates[TMYC, DECAY] * concentration[TMYC];
        # A04. tMyc ----> Myc : Synthesis by translation
        ...
        # A05. 0 --IRF4--> Myc : Induction by IRF4
        reactionFlux[MYC, SYNTHESIS] += 0;
        ...
        #----------------------------------------
        # B01*. 0 --nNFkB--> tMyc activity : Activation by NFkB
        reactionFlux[TCYCD, ACTIVATION] = ((0.45*concentration[NA50] + 0.45*concentration[NC50] + 0.1*reactionFlux[IKK, ACTIVATION])/CYCDTHR)^2.0 / (1+((0.45*concentration[NA50] + 0.45*concentration[NC50] + 0.1*reactionFlux[IKK, ACTIVATION])/CYCDTHR)^2.0) * (1-Srates[TCYCD, ACTIVATION]) + Srates[TCYCD, ACTIVATION];
        # B02*. tMyc Act ----> tMyc : Synthesis
        reactionFlux[TCYCD, SYNTHESIS] = Srates[TCYCD, SYNTHESIS] * reactionFlux[TCYCD, ACTIVATION];
        # B03. tMyc ----> 0 : mRNA degradation
        reactionFlux[TCYCD, DECAY] = Srates[TCYCD, DECAY] * concentration[TCYCD];
        # B04. tMyc ----> Myc : Synthesis by translation
        reactionFlux[CYCD, TRANSLATION] = EPS * Srates[CYCD, TRANSLATION] * concentration[TCYCD];
        # B05. CycD ----> 0 : protein degradation
        reactionFlux[CYCD, DECAY] = Srates[CYCD, DECAY] * concentration[CYCD];
        # B06. CycD + p27 ----> CycD:p27 : complex formation
        reactionFlux[CYCDP27, ASSOCIATION] = Srates[CYCDP27, ASSOCIATION] * concentration[CYCD] * concentration[P27];
        # B07. CycD:p27 ----> CycD + p27 : complex dissociation
        reactionFlux[CYCDP27, DISSOCIATION] = Srates[CYCDP27, DISSOCIATION] * concentration[CYCDP27];
        # B08. CycD:p27 ----> p27 : protein degradation
...
function computeProlifNettFluxes!(nettFlux, concentration, reactionFlux, Srates, phase)
    ...
    @inbounds begin
        ...
        # 8* : tCycD
        nettFlux[TCYCD] = reactionFlux[TCYCD, SYNTHESIS] - reactionFlux[TCYCD, DECAY];
        # 8 : CycD
