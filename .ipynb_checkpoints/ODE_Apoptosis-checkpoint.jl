...
function computeApoptosisFluxes!(concentration, reactionFlux, Srates, phase)
    @inbounds begin
        ...
        #--------------------------------------------
        # MODULE 1: NF-kB regulatory system
        #--------------------------------------------
        ...
        reactionFlux[TBCL2, ACTIVATION] = ((0.3*concentration[NA50] + 0.6*concentration[NC50] + 0.1*reactionFlux[IKK, ACTIVATION])/BCL2THR)^2.0 / (1+((0.3*concentration[NA50] + 0.6*concentration[NC50] + 0.1*reactionFlux[IKK, ACTIVATION])/BCL2THR)^2.0) * (1-Srates[TBCL2, ACTIVATION]) + Srates[TBCL2, ACTIVATION];
        ...
        reactionFlux[TPARP, BASALDECAY] = Srates[TPARP, BASALDECAY] * concentration[TPARP];
...
function computeApoptosisNettFluxes!(nettFlux, concentration, reactionFlux, Srates, phase)
    ...
    @inbounds begin
        ...
        #--------------------------------------------
        # 8 : pC8
        nettFlux[PC8] = reactionFlux[PC8, BASALSYNTHESIS] - reactionFlux[PC8, BASALDECAY] - reactionFlux[DISCPC8, ASSOCIATION] + reactionFlux[DISCPC8, DISSOCIATION] - reactionFlux[C6PC8, ASSOCIATION] + reactionFlux[C6PC8, DISSOCIATION];
        # 9 : DISC:pC8
        nettFlux[DISCPC8] = reactionFlux[DISCPC8, ASSOCIATION] - reactionFlux[DISCPC8, DISSOCIATION] - reactionFlux[C8, ACTIVATION];
        # 10 : C8
        nettFlux[C8] = reactionFlux[C8, ACTIVATION] - reactionFlux[BARC8, ASSOCIATION] + reactionFlux[BARC8, DISSOCIATION] - reactionFlux[C8BID, ASSOCIATION] + reactionFlux[C8BID, DISSOCIATION] + reactionFlux[TBID, ACTIVATION] - reactionFlux[C8PC3, ASSOCIATION] + reactionFlux[C8PC3, DISSOCIATION] + reactionFlux[C3, ACTIVATION] + reactionFlux[C8, SYNTHESIS];
        # 11 : Bar
        ...
        # 21 : mBax
        nettFlux[MBAX] = reactionFlux[ABAX, TRANSPORTIN] - reactionFlux[MBAX, TRANSPORTOUT] - reactionFlux[MBAXBCL2, ASSOCIATION] + reactionFlux[MBAXBCL2, DISSOCIATION] - 2 * reactionFlux[BAX2, ASSOCIATION] + 2 * reactionFlux[BAX2, DISSOCIATION];
        # 22 : Bcl2
        nettFlux[BCL2] = reactionFlux[BCL2, TRANSLATION] - reactionFlux[BCL2, BASALDECAY] - reactionFlux[MBAXBCL2, ASSOCIATION] + reactionFlux[MBAXBCL2, DISSOCIATION] - reactionFlux[BAX2BCL2, ASSOCIATION] + reactionFlux[BAX2BCL2, DISSOCIATION] - reactionFlux[BAX4BCL2, ASSOCIATION] + reactionFlux[BAX4BCL2, DISSOCIATION];
        # 23 : mBax:Bcl2
        nettFlux[MBAXBCL2] = reactionFlux[MBAXBCL2, ASSOCIATION] - reactionFlux[MBAXBCL2, DISSOCIATION] - reactionFlux[MBAXBCL2, BASALDECAY];
        # 24 : Bax2
        nettFlux[BAX2] = reactionFlux[BAX2, ASSOCIATION] - reactionFlux[BAX2, DISSOCIATION] - reactionFlux[BAX2BCL2, ASSOCIATION] + reactionFlux[BAX2BCL2, DISSOCIATION] - 2 * reactionFlux[BAX4, ASSOCIATION] + 2 * reactionFlux[BAX4, DISSOCIATION];
        # 25 : Bax2:Bcl2
        nettFlux[BAX2BCL2] = reactionFlux[BAX2BCL2, ASSOCIATION] - reactionFlux[BAX2BCL2, DISSOCIATION] - reactionFlux[BAX2BCL2, BASALDECAY];
        # 26 : Bax4
        nettFlux[BAX4] = reactionFlux[BAX4, ASSOCIATION] - reactionFlux[BAX4, DISSOCIATION] - reactionFlux[BAX4BCL2, ASSOCIATION] + reactionFlux[BAX4BCL2, DISSOCIATION] - reactionFlux[BAX4MITO, ASSOCIATION] + reactionFlux[BAX4MITO, DISSOCIATION];
        # 27 : Bax4:Bcl2
        nettFlux[BAX4BCL2] = reactionFlux[BAX4BCL2, ASSOCIATION] - reactionFlux[BAX4BCL2, DISSOCIATION] - reactionFlux[BAX4BCL2, BASALDECAY];

        #------------------------------------------------
        # MODULE 6
        #--------------------------------------------
        # 28 : Mito
        nettFlux[MITO] = - reactionFlux[BAX4MITO, ASSOCIATION] + reactionFlux[BAX4MITO, DISSOCIATION] + reactionFlux[AMITO, DEACTIVATION];
        # 29 : Bax4:Mito
        nettFlux[BAX4MITO] = reactionFlux[BAX4MITO, ASSOCIATION] - reactionFlux[BAX4MITO, DISSOCIATION] - reactionFlux[AMITO, ACTIVATION];
        # 30 : aMito
        nettFlux[AMITO] = reactionFlux[AMITO, ACTIVATION] - reactionFlux[AMITOMCYTOC, ASSOCIATION] + reactionFlux[AMITOMCYTOC, DISSOCIATION] + reactionFlux[ACYTOC, ACTIVATION] - reactionFlux[AMITOMSMAC, ASSOCIATION] + reactionFlux[AMITOMSMAC, DISSOCIATION] + reactionFlux[ASMAC, ACTIVATION] - reactionFlux[AMITO, DEACTIVATION];
        # 31 : mCytoC
        nettFlux[MCYTOC] = reactionFlux[MCYTOC, BASALSYNTHESIS] - reactionFlux[MCYTOC, BASALDECAY] - reactionFlux[AMITOMCYTOC, ASSOCIATION] + reactionFlux[AMITOMCYTOC, DISSOCIATION];
        # 32 : aMito-mCytoC
        nettFlux[AMITOMCYTOC] = reactionFlux[AMITOMCYTOC, ASSOCIATION] - reactionFlux[AMITOMCYTOC, DISSOCIATION] - reactionFlux[ACYTOC, ACTIVATION];
        # 33 : aCytoC
        nettFlux[ACYTOC] = reactionFlux[ACYTOC, ACTIVATION] - reactionFlux[ACYTOC, TRANSPORTOUT] + reactionFlux[CCYTOC, TRANSPORTIN];
        # 34 : mSmac
        nettFlux[MSMAC] = reactionFlux[MSMAC, BASALSYNTHESIS] - reactionFlux[MSMAC, BASALDECAY] - reactionFlux[AMITOMSMAC, ASSOCIATION] + reactionFlux[AMITOMSMAC, DISSOCIATION];
        # 35 : aMito:mSmac
        nettFlux[AMITOMSMAC] = reactionFlux[AMITOMSMAC, ASSOCIATION] - reactionFlux[AMITOMSMAC, DISSOCIATION] - reactionFlux[ASMAC, ACTIVATION];
        # 36 : aSmac
        ...
        # 59 : tPARP
        nettFlux[TPARP] = reactionFlux[PARP, DEACTIVATION] - reactionFlux[TPARP, BASALDECAY]
        #------------------------------------------------
