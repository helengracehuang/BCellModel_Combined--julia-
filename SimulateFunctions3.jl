# 3rd edition of reaction rate parameters (added support for receptor model & simulate NFkB and Apoptosis separately)
using Base.Threads;
...
function Simulate!(allCells, Srates, i, delay)
    @inbounds begin
        ...
        solution = solve(Bcell2, method, callback=cellFate, abstol=abserr, reltol=relerr, save_everystep=false, maxiters=1e10);
        ...
        if solution.u[end][CPARP]/solution.u[end][PARP] ≈ 1.0 || solution.u[end][CPARP] ≈ 2500.0   # death (for symplified or full apoptosis model)
            allCells[i].fate = 1;
        ...
        elseif NFkBsolution.u[end][IRF4] - 0.65*NFkBsolution.u[end][BCL6] ≈ 1.2   # differentiation
            allCells[i].fate = 3;
            allCells[i].abs_fate_t = allCells[i].birthday + allCells[i].fate_t;
