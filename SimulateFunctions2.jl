...
#-------------------------------------------------------------------------
function Simulate!(allCells, Srates, i, NEMOCurve, NIKCurve, delay)
    @inbounds begin
        has_children = false;
        Bcell = DDEProblem(computeNetworkNettFluxes!, (@view allCells[i].rates[1:end, INITIALCONC]), delay, (0.0, GLOBAL_END_TIME-allCells[i].birthday), (allCells[i].birthday, NEMOCurve, NIKCurve, allCells[i].rates, allCells[i].reactionFlux, allCells[i].historicFlux); constant_lags=[0.25, 0.75, 1.0, 4.0, 12.0]);
        ...
        allCells[i].finalConcentration[1:NFKB_SPECIES+DIFF_SPECIES] .= solution.u[end];

        allCells[i].fate_t = solution.t[end];
        if solution.u[end][TPARP]/solution.u[end][PARP] ≈ 1.0   # death
            allCells[i].fate = 1;
...
#-------------------------------------------------------------------------
function Simulate_lineage!(allCells, Srates, i, NEMOCurve, NIKCurve, delay)
    stack = Int[i]
    while !isempty(stack)
        j = pop!(stack)
        has_children, daughter_1_idx, daughter_2_idx = Simulate!(allCells, Srates, j, NEMOCurve, NIKCurve, delay)
        if has_children
...
#-------------------------------------------------------------------------
function Simulate_nonthreaded!(allCells, Srates, NEMOCurve, NIKCurve, delay)
    @inbounds for i in 1:FOUNDER_CELL_NUM
        Simulate_lineage!(allCells, Srates, i, NEMOCurve, NIKCurve, delay)
    end
end

# Define function to simulate all cell lineages w/ multi-threading (static schedule)
#-------------------------------------------------------------------------
function Simulate_threaded!(allCells, Srates, NEMOCurve, NIKCurve, delay)
...
#-------------------------------------------------------------------------
function Simulate_spawned!(allCells, Srates, NEMOCurve, NIKCurve, delay)
    tasks = [Threads.@spawn(Simulate_lineage!(allCells, Srates, i, NEMOCurve, NIKCurve, delay)) for i in 1:FOUNDER_CELL_NUM]
